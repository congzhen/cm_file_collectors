...
const resourcesDirectorsServerData = {
    ...
    deleteDirectorsByResourcesId: async function (resources_id: string) {
        const delResult = await CoreDb().table('resourcesDirectors').where('resources_id', '=', resources_id).deleteWhere();
